...
impl Validator {
    ...
    pub fn new(consensus: Arc<Consensus>, validator_key: KeyPair) -> Result<Arc<Self>, Error> {
        ...
        let validator_network = ValidatorNetwork::new(
            ...
            SignedValidatorInfo::from_message(info, &validator_key.secret, 0),
        );
    ...
    fn start_view_change(&self) {
        ...
        let view_change_message =
            SignedViewChange::from_message(message.clone(), &self.validator_key.secret, pk_idx);
    ...
    fn produce_macro_block(
        ...
        let signed_proposal =
            SignedPbftProposal::from_message(pbft_proposal, &self.validator_key.secret, pk_idx);
        self.validator_network
