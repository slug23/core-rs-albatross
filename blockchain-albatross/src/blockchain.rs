...
impl Blockchain {
    ...
    fn rebranch(&self, block_hash: Blake2bHash, chain_info: ChainInfo) -> Result<PushResult, PushError> {
        ...
        while !current.1.on_main_chain {
            ...
            // A fork can only contain a macro block as its head (in case the fork ocurred in the block previous to the macro block),
            // so from this point on, no block should be a macro block
